...
class _NotifyingPageState extends State<NotifyingPage> {
  ...
  void makeNotify() async {
    BusService.instance?.addUser(selectedBusNumbers).then((value) async => {
          await NotificationService.instance?.sendNotificationToDrivers(
              selectedBusNumbers, "Passenger is waiting for you"),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      AnimatedContainer(
          ...
          child: const Image(
              image: AssetImage('assets/icons/locationicon.png'), width: 45)),
      SafeArea(
          child: AnimatedAlign(
              duration: const Duration(milliseconds: 300),
                          ...
                          onPressed: () => {
                            ...
                            NotificationService.instance
                                ?.sendNotificationToDrivers(selectedBusNumbers,
                                    "Passenger is on the bus"),
                            // Navigator.pop(context)
                            // Navigator.pushNamed(context, '/pwd-on-way-page'),
                            Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => PwdOnWayPage(
