...
class PwdOnWayPage extends StatefulWidget {
  ...
  const PwdOnWayPage(
      {super.key, required this.selectedBusNumbers, required this.busStations});

  @override
  State<PwdOnWayPage> createState() => _PwdOnWayPageState(
      selectedBusNumbers: selectedBusNumbers, busStations: busStations);
}
...
class _PwdOnWayPageState extends State<PwdOnWayPage> {
  ...
  _PwdOnWayPageState(
      {required this.selectedBusNumbers, required this.busStations});
    ...
    Timer.periodic(
        ...
        (Timer t) => selectedStation != busStations.length - 1
            ? Geolocator.getCurrentPosition()
                .then((value) => findTheNearest(value))
            : {
                TtsService.instance
                    ?.lastStopSpeach(busStations[selectedStation].name),
                t.cancel(),
              });
  }
        ...
        body: SafeArea(
          ...
          Container(
              ...
              decoration: const BoxDecoration(color: Colors.white, boxShadow: [
                BoxShadow(
                    color: Color.fromRGBO(0, 0, 0, .18), offset: Offset(0, 0.4))
              ]),
              child: const Row(
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                            ...
                            border: selectedStation == i
                                ? Border.all(
                                    color: const Color.fromRGBO(41, 86, 154, 1),
                                    width: 1.5)
                                : null),
                        child: selectedStation == i
                            ? const Icon(Ionicons.bus,
                                size: 13, color: Color.fromRGBO(41, 86, 154, 1))
                            : null,
                      ),
